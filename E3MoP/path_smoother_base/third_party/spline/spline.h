...
public:
    ...
    double get_x_min() const { CHECK(!m_x.empty()); return m_x.front(); }
    double get_x_max() const { CHECK(!m_x.empty()); return m_x.back(); }
...
{
    CHECK(m_x.size()==0);          // set_points() must not have happened yet
    m_left=left;
    m_right=right;
...
{
    CHECK(m_x.size()==m_y.size());
    CHECK(m_x.size()==m_b.size());
    CHECK(m_x.size()>2);
    size_t n=m_b.size();
    if(m_c.size()!=n)
        m_c.resize(n);
...
double & band_matrix::operator () (int i, int j)
{
    ...
    CHECK( (i>=0) && (i<dim()) && (j>=0) && (j<dim()) );
    CHECK( (-num_lower()<=k) && (k<=num_upper()) );
